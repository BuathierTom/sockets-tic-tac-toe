...
int main(int argc, char *argv[]){
	...
	char buffer[100]; // buffer stockant le message
	...
	lus = read(descripteurSocket, buffer, LG_MESSAGE*sizeof(char)); // ici appel bloquant
		switch(lus) {
			case -1 : /* une erreur ! */ 
				  perror("read"); 
				  close(descripteurSocket); 
				  exit(-5);
			case 0  : /* la socket est fermée */
				  fprintf(stderr, "La socket a été fermée par le serveur zbi !\n\n");
   				  close(descripteurSocket);
   				  return 0;
